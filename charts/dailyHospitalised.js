...
module.exports = (data) => {
  ...
  const options = {
    ...
    container: '<div id="container"><div id="hospitalised"></div></div>',
  };
  ...
  const hospitalData = data.hospital.map((d, i) => ({
    ...d,
    date: new Date(d.date),
    icuCases: data.icu[i - icuArrayLengthDiff]
      ? data.icu[i - icuArrayLengthDiff].icuCases
      : 0,
  }));

  // Prepare data to put inline
  const findByDate = (date, dataset) =>
    dataset.find((d) => {
  ...
  // ConfirmedCovidDeaths is weekly as of May 2021
  // let confirmedDeaths;
  ...
  // Draw containing svg
  const svg = d3
    .select(d3n.document.querySelector("#hospitalised"))
    .append("svg")
    .attr("viewBox", `0 0 ${w} ${h}`);

  // Draw axes
  const xAxis = d3.axisBottom(xScale).tickPadding(5).tickSize(5);
  ...
  svg.selectAll(".tick text").attr("fill", colours.darkGrey);

  // Define hospitalised as an area
  const hospitalised = d3
    .area()
    .curve(d3.curveBasis)
    .x((d) => xScale(d.date))
    .y0(() => yScale.range()[0])
