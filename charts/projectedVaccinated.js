...
module.exports = (data) => {
  ...
  const options = { selector: '#projected-vaccinations', container: '<div id="container"><div id="projected-vaccinations"></div></div>' };
  ...
  // Draw containing svg
  const svg = d3.select(d3n.document.querySelector('#projected-vaccinations'))
    .append('svg')
  ...
  const projectedDosesLine = d3.line()
    .x(d => xScale(d.x))
    .y(d => yScale(d.y));
